...
const canaryPlaintext = 'thunderbolt-key-check-v1'
...
/**
 * Encrypt canaryPlaintext with the given master key.
...
export const createCanary = async (masterKey: CryptoKey): Promise<KeyCanary> => {
  const plaintext = new TextEncoder().encode(canaryPlaintext)
 ...
 * Returns true if decryption succeeds and plaintext matches canaryPlaintext.
...
export const verifyCanary = async (masterKey: CryptoKey, canary: KeyCanary): Promise<boolean> => {
  try {
    ...
    const expected = new TextEncoder().encode(canaryPlaintext)
